...
func queuePayment(flow interface{}, queue string, order Order, user User) error {
	time.Sleep(10 * time.Second) // Simulating payment processing delay
...
func OrderWorkflow(ctx workflow.Context, order Order) error {
	...
	orderStatus = "waiting for delivery notification"
	...
	orderStatus = "notifying delivery"
	err = workflow.ExecuteActivity(ctx, notifyDelivery, order).Get(ctx, nil)
	if err != nil {
		return err
	}
...
func PaymentWorkflow(ctx workflow.Context, user User, order Order) error {
	...
	paymentStatus = "charging credit card"
	...
	paymentStatus = "notifying order workflow"
...
func main() {
	...
	order := Order{ID: 3, Product: "T-shirt", Quantity: 2}
	we, err := c.ExecuteWorkflow(context.Background(), client.StartWorkflowOptions{
		ID:        orderWorkflowId(order), // "order-1
	...
	//	resp, err := c.QueryWorkflow(context.Background(), orderWorkflowId(order), "", "state")
	//	if err != nil {
